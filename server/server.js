...
io.on("connection", (socket)=>{
    ...
    socket.on("guestLoginCredentials", (loginCredentials)=>{
        const {guestUsername, guestPassword} = loginCredentials;

        console.log("GuestUSer "+guestUsername);
        console.log("GuestPass "+guestPassword);

        user.findOne({username: guestUsername}).then((foundUser)=>{
            if(!foundUser){
                console.log("No User Found");
            ...
            {
                if(foundUser.password === guestPassword)
                {
                    console.log("Login Success");
                    usernametToSocketId[guestUsername]=socket.id;
    ...
    socket.on("endCall", (endCallDetails)=>{
        const {otherUser, username} = endCallDetails;

        io.to(usernametToSocketId[otherUser]).emit("endCall");
        io.to(usernametToSocketId[username]).emit("endCall");
    })
})